 ...
 *   - Los payloads que se pasan al driver deben ser (size-4) bytes y el puntero debe
 *     apuntar a subpack+4; justo lo que te entrega el callback.
 */
...
static int on_block(int pack, uint16_t id, uint16_t size, const uint8_t *payload, void *user) {
    struct opts *o = (struct opts*)user;

    if (o->filter_pack >= 0 && pack != o->filter_pack) return 0;
    if (o->filter_id   >= 0 && id   != (uint16_t)o->filter_id) return 0;

    switch (o->mode) {
        case 0: /* dump */
            printf("pack=%d id=0x%02x size=%u\n", pack, id, (unsigned)size);
            if (o->hexdump > 0 && payload && size > 0) {
                hexdump_cap(payload, size, o->hexdump, 0);
            } else if (o->hexdump == -1 && payload && size > 0) {
                hexdump_cap(payload, size, size - 4, 0);
            }
            break;
        case 1: /* apply-set (stub) */
            printf("pack=%d id=0x%02x size=%u -> set\n", pack, id, (unsigned)size);
            (void)ak_isp_apply_stub(1, id, payload, size);
            break;
        case 2: /* apply-get (stub) */
            printf("pack=%d id=0x%02x -> get\n", pack, id);
            (void)ak_isp_apply_stub(0, id, payload, size);
            break;
    }
    return 0;
}
